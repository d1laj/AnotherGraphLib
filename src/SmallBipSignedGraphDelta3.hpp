...
// TODO DataMember -> Compact DataMember

template <typename DataType> struct VertexDataViewBase { DataType data; };

template <typename DataType, int BitMin, int BitMax> struct DataMember {
  ...
  // operator =
  const DataMember &operator--(int) const;
  const DataMember &operator++() const;
  const DataMember &operator--() const;
  ...
  DataType val() const { return parent.data >> 5; }
  VertexDataViewBase<DataType> &parent;
  static const int bitMin = BitMin;
  static const int bitMax = BitMax;
  // static const mask1 , mask2;
  using dataType = DataType;
};

#define AGL_GENERATE_DATA_MEMBER(hasName, name)                                \
  template <typename DataType, typename T> struct hasName##Base {              \
    using DMType = T;                                                          \
    T name;                                                                    \
  ...
  using hasName =                                                              \
      hasName##Base<DataType, DataMember<DataType, BitMin, BitMax>>;
...
struct DataMemberList : DMArgs... {
  DataMemberList(VertexDataViewBase<DataType> &p) : DMArgs(p)... {
    static_assert(
        valid(),
        "Invalid DataMemberList: overlaping between the data members.");
  }
...
struct VertexDataView : VertexDataViewBase<DataType> {
  struct Deg {
    ...
    u8 val() const { return parent.data >> 5; }
